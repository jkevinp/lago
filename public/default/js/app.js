...
$(document).ready(function() {
    ...
    $( "body" ).delegate( ".dynamic-form", "submit", function(event) {
        ...
        if(frm.attr("files") == "true"){
            $('input[type="file"]').each(function(index ,object){
                ...
                else post_data.append($(object).attr("name"), file_data[0]);
                }
        ...
        if(error){
            ...
            $.ajax({
                ...
                success : function(data) {    
                    ...
                    if (!data.status) { //if status is false
                        var url  = "";
                        ...
                        if(data.message){
                            ...
                            m_error2(data.message, function(){ 
                                ...
                                log("others: "+ data.others);
                                if(typeof data.others !== "undefined"){
                                    handleResponse(data.others , url);
                                }
                                else if(typeof data.others === "undefined" && url.length){
                                    redirect(url);
                                }
                                else{
                                    log("ELSE");
                ...
                error: function(xhr, status, error) {
                    console.log("error on dynamic form");
    ...
    $( "body" ).delegate( ".a-confirmation", "click", function(event) {
        ...
        event.preventDefault();
        m_alert2( trans('are_you_sure_continue'), function(){
            redirect(url);
        },function(){
            m_info(trans('cancelled'));
        });
    });
  ...
  $('.loader').each(function(index, object){
    ...
    $( "body" ).delegate( ".ajaxget-confirmation", "click", function(event) {
     ...
     $( "body" ).delegate( ".ajaxpost-confirmation", "click", function(event) {
      ...
      $( "body" ).delegate( ".form-confirmation", "click", function(event) {
        event.preventDefault();
        var url = $(this).attr("href");
        var me = $(this);
        var oldbuttonhtml = me.html();
